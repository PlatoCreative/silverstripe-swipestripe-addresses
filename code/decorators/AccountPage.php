    ...
    */
    public function deleteAddress(){
        ...
        if($_POST){
            ...
            if(!isset($_POST['addressID']) || !isset($_POST['type'])){
                return $this->owner->httpError(403, 'No Address found or you are not allowed to delete that address.');
            }
    ...
    */
    public function addAddress($request){
        if($_POST){
            // is there an ID to process or type?
            if(!isset($_POST['type'])){
                return $this->owner->httpError(403, 'No Address type passed you are not allowed to add an address.');
            }
            ...
            if(Member::currentUserID()){
                ...
                if($type == "shipping"){
                    $newAddress = Address_Shipping::create(array(
                        ...
                        'Default' => (isset($data['ShippingDefault'])) ? $data['ShippingDefault'] : null,
                    ));
                    ...
                    $newAddress = Address_Billing::create(array(
                        ...
                        'Default' => (isset($data['BillingDefault'])) ? $data['BillingDefault'] : null
                    ));
                ...
                $returnAddress['Default'] = $newAddress->Default;
    ...
    */
    public function editAddressAction(){
        if($_POST){
            // is there an ID to process or type?
            if(!isset($_POST['type'])){
                return $this->owner->httpError(403, 'No Address type passed you are not allowed to edit an address.');
            }
